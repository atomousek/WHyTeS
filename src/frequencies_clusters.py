...
class Frequencies:
    ...
    def _get_params(self, path):
        ...
        X = tr.create_X(self._get_data(path), self.structure)
        start = time()
        clf = hdbscan.HDBSCAN(min_cluster_size=4000, min_samples=1).fit(X)  # min_cluster_size is to be reconsidered
        finish = time()
        print('time for clustering: ' + str(finish-start))
        labels = clf.labels_
        self.clusters = len(set(labels)) -1
        PREC, C = self._calc_precisions(X, labels)
        labels = labels[labels != -1]
        unique_labels, U = np.unique(labels, return_counts=True)
        print U 
        return C, U, PREC
    ...
    def _calc_precisions(self, X, labels):
        labeled_data = np.c_[X, labels]
        unique_labels = set(labels)
        precs = []
        centers = []
        if -1 in unique_labels:
            unique_labels.remove(-1) # filter noise points
        print len(unique_labels)
        for label in unique_labels:
            cluster = labeled_data[labeled_data[:, -1] == label]
            center = []

            for i in range(labeled_data.shape[1] - 1):
                mean = np.mean(cluster[:, i]) 
                center.append(mean)
