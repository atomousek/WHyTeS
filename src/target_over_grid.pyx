...
cdef inline void rounding_and_indexes(const double[:,:] dataset, const double[:] edges, const long [:] no_bins, const double [:] starting_points, const long rows, const long columns, double [:] out, double [:] indexes, long len_out_minus, double [:] count) nogil:
