...
@cython.boundscheck(False)
@cython.wraparound(False)
def generate(double [:] edges, long [:] no_bins, double [:] starting_points, long dim):#, double [:,:] out):
    cdef long counter = 0
    cdef cnp.ndarray[cnp.float64_t, ndim=1] bin_centre = np.zeros(dim, dtype=np.float64)
    cdef long number_of_cells_for_no_use
    cdef long i
    cdef long length_of_out = 1
    for i in no_bins:
        length_of_out *= i
    cdef cnp.ndarray[cnp.float64_t, ndim=2] out = np.zeros((length_of_out, dim), dtype=np.float64)
